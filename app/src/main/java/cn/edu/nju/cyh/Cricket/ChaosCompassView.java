...
public class ChaosCompassView extends View {
    ...
    private int mOutSideRadius;/*外圆半径*/
    private int mCircumRadius;/*外接圆半径*/
    private int mTextHeight;/*指南针文字大小空间高度*/
    //画笔
    private Paint mDarkRedPaint;/*暗红色 外圈笔*/
    private Paint mDeepGrayPaint;/*深灰 外圈笔*/
    private Paint mOutSideCircumPaint;/*外三角笔*/
    private Paint mLightGrayPaint;/*浅灰 外圈笔*/
    private Paint mTextPaint;/*指南针上面 文字笔*/
    private Paint mCircumPaint;/*//外接圆，三角形笔*/
    //文字居中
    private Rect mTextRect;/*指南针上面文字的外接矩形,用来测文字大小让文字居中*/
    //绘画路径
    private Path mOutsideTriangle;/*外圈小三角形的Path*/
    private Path mCircumTriangle;/*外接圆小三角形的Path*/
    //NESW 文字笔 和文字外接矩形
    ...
    private void drawGuideLine(){
        ...
        mGuidePaint.setColor(Color.CYAN);
        mGuidePaint.setStyle(Paint.Style.STROKE);
        mGuidePaint.setStrokeWidth(10);

        Path GuideLine=new Path();
        mCanvas.drawCircle(width/2,mOutSideRadius+mTextHeight,1,mGuidePaint);

        Double d1=new Double(mOutSideRadius*Math.cos(Math.PI*mdegree/180));
        Double d2=new Double(mOutSideRadius*Math.sin(Math.PI*mdegree/180));
        mGuidePaint.setStrokeWidth(3);
        GuideLine.moveTo(width/2,mOutSideRadius+mTextHeight);
        GuideLine.lineTo(width/2+d1.floatValue(),mOutSideRadius+mTextHeight-d2.floatValue());
        GuideLine.close();
        mCanvas.drawPath(GuideLine,mGuidePaint);

        int mTriangleHeight=30;
        float mTriangleSide = 36.18f;
        Double dTriangle11=new Double((mOutSideRadius+mTriangleHeight)*Math.cos(Math.PI*mdegree/180));
        Double dTriangle12=new Double((mOutSideRadius+mTriangleHeight)*Math.sin(Math.PI*mdegree/180));
        Float dtan=new Float(mTriangleSide/(2*mOutSideRadius));
        double tanVal=Math.atan(dtan)*180/Math.PI;
        Double dTriangle21=new Double(mOutSideRadius*Math.cos(Math.PI*(mdegree-tanVal)/180));
        Double dTriangle22=new Double(mOutSideRadius*Math.sin(Math.PI*(mdegree-tanVal)/180));
        Double dTriangle31=new Double(mOutSideRadius*Math.cos(Math.PI*(mdegree+tanVal)/180));
        Double dTriangle32=new Double(mOutSideRadius*Math.sin(Math.PI*(mdegree+tanVal)/180));
        GuideLine.moveTo(width/2+dTriangle11.floatValue(),mOutSideRadius+mTextHeight-dTriangle12.floatValue());
        GuideLine.lineTo(width/2+dTriangle21.floatValue(),mOutSideRadius+mTextHeight-dTriangle22.floatValue());
        GuideLine.lineTo(width/2+dTriangle31.floatValue(),mOutSideRadius+mTextHeight-dTriangle32.floatValue());
        GuideLine.close();
        mCanvas.drawPath(GuideLine,mGuidePaint);
